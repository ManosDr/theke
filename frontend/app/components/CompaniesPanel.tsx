...
export function CompaniesPanel() {
  ...
  // Deep-link support for AdminUsersPanel's per-user "usage" row action
  // (?company=<id>) - lets it link straight into the existing per-company
  // usage view (CompanyDetailModal's token_usage breakdown) instead of that
  // screen needing its own separate usage display.
  ...
  useEffect(() => {
    ...
    api.get<CompanyDetail>(`/admin/companies/${companyId}`, token).then(setDetail).catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams, token]);
