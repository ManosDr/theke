...
Υπεύθυνος επεξεργασίας των προσωπικών δεδομένων που συλλέγονται μέσω της πλατφόρμας Theke είναι η **[ΕΠΩΝΥΜΙΑ ΙΚΕ, ΑΦΜ, ΓΕΜΗ, ΕΔΡΑ]** («εμείς», «η Θήκη»).
...
### 2.2 Ερωτήματα / Chat
Κάθε ερώτηση που υποβάλλετε στη Θήκη αποθηκεύεται στη βάση δεδομένων μας και αποστέλλεται στον πάροχο τεχνητής νοημοσύνης OpenAI για την παραγωγή απάντησης (βλ. Ενότητα 4).
...
| Στοιχεία πελατών σας (ΑΦΜ κ.λπ.) | Λειτουργία διαχείρισης υποθέσεων εντός πλατφόρμας | Εκτέλεση σύμβασης μεταξύ εσάς και της Θήκης· εσείς φέρετε τη δική σας νομική βάση έναντι των πελατών σας |
...
Η Θήκη χρησιμοποιεί το μοντέλο τεχνητής νοημοσύνης **GPT-4o της εταιρείας OpenAI** (έδρα: ΗΠΑ) για να επεξεργαστεί κάθε ερώτημα που υποβάλλετε και να παράγει την απάντηση με παραπομπές. Αυτό σημαίνει πρακτικά:

- **Κάθε ερώτηση που κάνετε στη Θήκη, και το κείμενο κάθε εγγράφου που ανεβάζετε (για τη δημιουργία embeddings), αποστέλλεται στους διακομιστές του OpenAI στις ΗΠΑ για επεξεργασία.** Αυτό δεν είναι προαιρετικό — είναι ο τρόπος που λειτουργεί ο πυρήνας της Υπηρεσίας.
- Ο OpenAI ενεργεί ως **Εκτελών την Επεξεργασία (data processor)** για λογαριασμό της Θήκης, όχι ως ανεξάρτητος Υπεύθυνος Επεξεργασίας. Αυτό σημαίνει ότι δεν έχει το δικαίωμα να χρησιμοποιήσει τα δεδομένα σας για δικούς του σκοπούς.
...
> 💬 **Πρακτικό βήμα για εσένα, όχι απλώς κείμενο:** Η Θήκη πρέπει να έχει **υπογράψει επίσημα** το Data Processing Addendum του OpenAI μέσω του λογαριασμού OpenAI API (βρίσκεται στις ρυθμίσεις του οργανισμού σου στο platform.openai.com). Αυτό δεν γίνεται αυτόματα — απαιτεί ενεργή αποδοχή. Αν δεν το έχεις κάνει ακόμα, είναι προτεραιότητα πριν δημοσιεύσεις αυτή την Πολιτική, γιατί το κείμενο παρακάτω προϋποθέτει ότι υπάρχει.
...
Αν είστε λογιστής/φοροτεχνικός και καταχωρείτε στοιχεία (ονοματεπώνυμο, ΑΦΜ) των δικών σας πελατών στη Θήκη:
...
- **Η Θήκη ενεργεί ως Εκτελών την Επεξεργασία** για αυτά τα δεδομένα, στο πλαίσιο μιας Σύμβασης Επεξεργασίας Δεδομένων (Data Processing Agreement) μεταξύ της Θήκης και εσάς, σύμφωνα με το άρθρο 28 ΓΚΠΔ.
