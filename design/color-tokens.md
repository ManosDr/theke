...
**58 tokens total** (up by 2 since the last export — see changelog below).

## What changed since the last export

- **Bug fix, not a rename:** the previous export silently dropped `chat.citation_chip_bg`/`chat.citation_chip_text` with no changelog entry. Those two documented the pre-v2 numbered citation badge (markup was `.citations li`/`.citationLink`, itself an alias of `core.background.surface_alt`/`core.text.muted`). The v2 chat redesign replaced that markup with `.sourceRow`/`.sourceBadge`/`.sourceTag` — two DIFFERENT sibling elements: `.sourceBadge` (the numbered badge, e.g. "1") and `.sourceTag` (the source-type pill, e.g. "ΤΕΕ"). `chat.citation_tag_bg`/`chat.citation_tag_bg_hover` (added in the same earlier pass, see next bullet) correctly document `.sourceTag`/`.sourceRow:hover` — they were never a replacement for `citation_chip_bg`/`text`, despite landing in the same spot in the file. `.sourceBadge` itself moved to a *different* pair, `--color-accent-soft-bg`/`--color-accent-soft-text` — declared in `globals.css` since the original v2 redesign commit but never added to this export on any prior pass. That's the real root cause: when `.sourceBadge` absorbed this already-untracked pair's styling, there was no entry to redirect, so its color silently fell out of the export instead of being reassigned. **Fixed** by adding `core.brand.accent_soft_bg`/`accent_soft_text` below, with `.sourceBadge` (and its two sibling call sites) listed as usage.
- Chat v2 redesign follow-up: added `chat.citation_tag_bg`/`chat.citation_tag_bg_hover` as two genuinely new tokens (`#eef0f3`/`#f3f5f9`) — the source-tag pill and source-row hover were both previously aliasing `core.background.surface_alt`, which read close to but not exactly the v2 design's two distinct values. Converted `chat.disclaimer_bg`/`disclaimer_border`/`disclaimer_text` from aliases of the shared `core.status.warning_bg`/`admin.status.warning_border`/`core.text.muted` into dedicated chat-only tokens matching the v2 design's exact values (`#fbf6e9`/`#efe4c4`/`#7a6425`) — repointing the shared warning tokens themselves would have repainted every other warning banner/badge app-wide. Folded the separate `chat.disclaimer_icon` token into `chat.disclaimer_text` (one warm-brown tone now covers both, per the v2 spec). Scoped to the desktop `.disclaimerBar` only — `.disclaimerMobile` is out of scope for this pass.
- **Also fixed:** the previous export's net-count math was wrong (claimed "57 → 61", actual result was 56) and this file's own header still said "61 tokens total" while its token table below still listed `citation_chip_bg`/`text` as if they and `citation_tag_bg`/`text` coexisted — both files are now regenerated together from one pass specifically so the stated count, the JSON, and this table can't drift apart again.
- **Flagged, not merged** (design-review decision, not this pass's call): `core.brand.accent` and `core.status.warning` hold the identical hex in BOTH themes (`#ffa800`/`#ffcf00`). Unlike the `--admin-danger`/`--error-red` and `--admin-info`/`--color-primary-hover` merges earlier in this file's history, these are deliberately left as two separate tokens here — a rebrand could want the brand accent and a semantic warning color to diverge even though they coincide today.
...
| `core.brand.accent_soft_bg` 🆕 | <span style="display:inline-block;width:14px;height:14px;background:#eaf0fb;border:1px solid #ccc;vertical-align:middle;"></span> `#eaf0fb` | <span style="display:inline-block;width:14px;height:14px;background:#22304c;border:1px solid #ccc;vertical-align:middle;"></span> `#22304c` | Soft-blue fill for small accent chips: assistant-identity avatar, numbered source-citation badge, active-project chip. Existed in `globals.css` since the v2 chat redesign but never tracked here until this fix (see changelog) |
| `core.brand.accent_soft_text` 🆕 | <span style="display:inline-block;width:14px;height:14px;background:#2f4b86;border:1px solid #ccc;vertical-align:middle;"></span> `#2f4b86` | <span style="display:inline-block;width:14px;height:14px;background:#9db4e8;border:1px solid #ccc;vertical-align:middle;"></span> `#9db4e8` | Text/icon color paired with `core.brand.accent_soft_bg` |
...
| `chat.citation_tag_bg` 🆕 | <span style="display:inline-block;width:14px;height:14px;background:#eef0f3;border:1px solid #ccc;vertical-align:middle;"></span> `#eef0f3` | <span style="display:inline-block;width:14px;height:14px;background:#1e2a44;border:1px solid #ccc;vertical-align:middle;"></span> `#1e2a44` | Source-tag pill (e.g. "ypen") background — was aliasing `core.background.surface_alt`, now its own token per the v2 chat redesign |
| `chat.citation_tag_bg_hover` 🆕 | <span style="display:inline-block;width:14px;height:14px;background:#f3f5f9;border:1px solid #ccc;vertical-align:middle;"></span> `#f3f5f9` | <span style="display:inline-block;width:14px;height:14px;background:#24334f;border:1px solid #ccc;vertical-align:middle;"></span> `#24334f` | Source-row hover background — deliberately distinct from `chat.citation_tag_bg` above |
| `chat.disclaimer_bg` ✏️ changed | <span style="display:inline-block;width:14px;height:14px;background:#fbf6e9;border:1px solid #ccc;vertical-align:middle;"></span> `#fbf6e9` | <span style="display:inline-block;width:14px;height:14px;background:#3a2f0a;border:1px solid #ccc;vertical-align:middle;"></span> `#3a2f0a` | Persistent top disclaimer bar background — was an alias of `core.status.warning_bg`, now a dedicated chat-only token matching the v2 design exactly |
| `chat.disclaimer_border` ✏️ changed | <span style="display:inline-block;width:14px;height:14px;background:#efe4c4;border:1px solid #ccc;vertical-align:middle;"></span> `#efe4c4` | <span style="display:inline-block;width:14px;height:14px;background:#5a4a1a;border:1px solid #ccc;vertical-align:middle;"></span> `#5a4a1a` | Disclaimer bar bottom border — dedicated token, see `disclaimer_bg` |
| `chat.disclaimer_text` ✏️ changed | <span style="display:inline-block;width:14px;height:14px;background:#7a6425;border:1px solid #ccc;vertical-align:middle;"></span> `#7a6425` | <span style="display:inline-block;width:14px;height:14px;background:#d9c48f;border:1px solid #ccc;vertical-align:middle;"></span> `#d9c48f` | Disclaimer bar copy + icon color — now also covers the icon (the separate `disclaimer_icon` token below was folded in, see changelog) |

*(`chat.disclaimer_icon` was removed — folded into `chat.disclaimer_text`, see changelog. `.disclaimerMobile`, out of scope for this pass, still uses the shared `core.status.warning_bg`/`admin.status.warning_border`/`core.text.muted` tokens, not these dedicated chat tokens. `chat.citation_chip_bg`/`chat.citation_chip_text` were removed — not a like-for-like swap with `citation_tag_bg`/`bg_hover` above, which document a different element; see the changelog's "Bug fix, not a rename" entry. The color role they used to document now lives at `core.brand.accent_soft_bg`/`accent_soft_text`.)*
