...
See [construction-ai-platform-blueprint.md](construction-ai-platform-blueprint.md) for the full roadmap, [KNOWN_DECISIONS.md](KNOWN_DECISIONS.md) for judgment calls made along the way that are deliberate but worth revisiting under specific conditions, [PROJECT_STATE.md](PROJECT_STATE.md) for a point-in-time, codebase-verified snapshot of exact schema, data, and test coverage, and [CAPABILITIES.md](CAPABILITIES.md) for a plain-language inventory of what the app actually does and doesn't do today - written as a testing brief, distinguishing real bugs from disclosed, honest stubs (e.g. exactly which of Greece's regions have real content vs. none).
