...
## Confirmed open, backend-owned: the model fabricates a bracketed pseudo-citation for facts drawn from the injected location-context block
...
**Not fixed here** - this was a verification pass (confirm whether a previously-unconfirmed report reproduces), not a fix task. A real fix needs a scoped decision, not a guess: either give the location-context block its own numbered marker in `_build_context_block()`'s scheme (treating it as a pseudo-source), or explicitly instruct the model in the system prompt to state location facts without a bracket citation at all (since they're project metadata, not a retrieved source, and don't need the same evidentiary marker). Backend/prompt-owned - not a frontend or citation-rendering bug, since the bracket text itself is inside `raw_answer` before it ever reaches the frontend.

**Revisit when:** this fix is actually prioritized - reproducible on demand (2/4 test questions against a real archaeological-flagged project), not a rare edge case, so it's a real, addressable target whenever picked up.
