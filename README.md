...
A fixed left sidebar (Dashboard / Sources / Search / Chat) plus a top header (page title, a search bar that works from any page, a notification bell, and the account menu) frame every page. For a super admin, the sidebar additionally shows the vertical switcher and an expandable nav tree (Γνωσιακή Βάση → Έγγραφα/Πηγές Δεδομένων, Εταιρείες, Ρυθμίσεις Συστήματος → Verticals & Content). The visual design - color palette, gradients, card/badge styling - is modeled on a reference dashboard design, recreated for both light and dark mode with the same layout in each; the admin screens layer a second, separate token set (navy/stone/parchment plus per-vertical green/bronze accents, from a later design handoff) on top of that for their cards/tables/badges specifically, while keeping the existing sidebar gradient and top bar unchanged.
