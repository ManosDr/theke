...
## Second occurrence of test-write pollution on a real-looking demo account - process gap, not a one-off

`backend/tests/test_admin.py::test_vertical_content_edit` sends a real `POST /chat/message` (to prove a disclaimer-text override actually reaches the answer) that gets permanently logged to `chat_sessions` regardless of test context - the test's `finally` block restored the vertical's `disclaimer_text` but never deleted that row. Confirmed via direct DB inspection: 201 stray rows had accumulated under `demo-member@accounting.theke.gr`'s company across repeated pytest runs over this project's history, each ending in a `"TEST DISCLAIMER <hex>"` placeholder instead of a real answer - discovered only by chance, in a UI screenshot, not by any test or monitoring. Fixed this one test's teardown (deletes its own row by `session_id`) and deleted the 201 existing rows directly.

This is the second time this exact failure shape has hit a real-looking demo account: a test performs a real write against shared/persistent state with no guarantee it's undone. The first time (an earlier session in this project) was a disclaimer field overwritten by a manual verification run with no snapshot taken first. Both times the fix was a per-test patch, not a structural one - nothing currently prevents a third occurrence somewhere else in the suite.

**Options worth evaluating, not yet decided:**
1. Run destructive/write-path tests against a dedicated, clearly-tagged test tenant, never a real demo account any human might log into for manual QA.
2. Wrap each test (or the whole suite) in a DB transaction that rolls back on teardown, so a forgotten cleanup step can't leave a trace no matter what the test does.
3. A lighter automatic sweep: a naming/marker convention for test-created rows (e.g. a `created_by_test=true` column, or the `"TEST "`-prefixed marker already used ad hoc here) that a scheduled job or a pytest session-teardown fixture sweeps automatically, as a backstop behind per-test cleanup rather than a replacement for it.

**Revisit when:** before the next test that needs to send a real chat message or otherwise write through a production code path in this suite - worth deciding the structural fix before a third instance, not after.

## Permanently-skipped `test_project_scoped_doc_isolated` is a dependency for the deferred customer-vs-project scoping architecture work

`tests/test_visibility.py::test_project_scoped_doc_isolated` is the only automated regression check for project-scoped document isolation - the single highest-stakes area in this project (cross-client document leakage). It's skipped via `@pytest.mark.skip` for a documented, verified-legitimate reason: its assertion sits downstream of a temperature=0 LLM off-topic classification call that has been observed to non-deterministically pass/fail on identical input, and the underlying retrieval/visibility logic was independently confirmed correct via a direct `_retrieve()` bypass call (see the test's own docstring). Not asking to unskip it now - the reasoning holds.

But this sits in the same territory as the still-open, explicitly-deferred customer-vs-project document scoping architecture decision (see the "Document scoping" entries earlier in this file). Once that architecture work lands, it will need a reliable automated regression check for cross-client leakage - and the only one that currently exists for this exact risk is the one marked skip. **Revisit when:** the customer-vs-project scoping architecture work is scheduled - resolve (or replace with a deterministic equivalent that doesn't depend on an LLM classification call) this skip before that work lands, not after, since it's the only existing safety net for the risk that work will be touching directly.
