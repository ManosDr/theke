...
**61 tokens total** (up by 4 since the last export — see changelog below).
...
*(`chat.disclaimer_icon` was removed — folded into `chat.disclaimer_text`, see changelog. `.disclaimerMobile`, out of scope for this pass, still uses the shared `core.status.warning_bg`/`admin.status.warning_border`/`core.text.muted` tokens, not these dedicated chat tokens.)*
