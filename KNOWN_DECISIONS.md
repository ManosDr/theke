...
**Still open, deliberately not decided here:** whether to go further with a structural backstop (a DB-transaction-rollback wrapper, or an automatic marker-sweep job) behind the per-test discipline now in place, and whether the 1331 historical rows in `test_chat.py`'s two demo accounts are worth a more careful (not blind exact-match) cleanup pass. Both are process-level calls for whoever owns test infrastructure decisions on this project, not something to guess at unilaterally.
