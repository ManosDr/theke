...
**Consequence for the build:** `lookup_cadastral_parcel()` and `lookup_gis_zone()` (`backend/app/services/gis.py`) are honest stubs that return `available: False` without attempting a call known to fail - they're not "not yet implemented" placeholders, they're a permanent reflection of a real external limitation. `check_archaeological_flag()` never calls a live API at all; it queries the existing RAG/KB infrastructure instead, sidestepping the JS-only Archaeological Cadastre portal entirely.

**`check_archaeological_flag()`'s real limitation: municipality-level granularity, not neighbourhood-level.** The only archaeological-zone content in the KB is the Παναγία/Kavala manual_entry document (the Παναγία historic peninsula, not all of Kavala). Since the function only receives a municipality name (from Nominatim's reverse-geocode result, not a finer-grained locality), any coordinate resolving to "Δήμος Καβάλας" - not just Παναγία specifically - will flag `archaeological_flag: true`, because that's genuinely the only archaeological-zone match available for that municipality. This was surfaced directly during Phase 5 browser testing: Nominatim's `quarter` field for the exact Panagia test coordinates (40.9389, 24.4131) came back as "Χωράφα", not "Παναγία" - OSM's neighbourhood boundaries don't line up with the informally-named historic area, so relying on the geocoder's own locality field for precision wasn't reliable either. The query template that feeds the RAG check (`αρχαιολογικοί περιορισμοί δόμησης {municipality}`, after stripping the generic "Δήμος"/"Δήμου" prefix) was tuned empirically - a bare "αρχαιολογική ζώνη {municipality}" scored measurably worse (0.549 vs. 0.435 cosine distance) against the same document. Accepted as a reasonable approximation given the asymmetric cost (a false "please double-check with ΥΠΠΟΑ" is a minor inconvenience; a missed real archaeological constraint is a substantive omission) and the confirmed absence of any real zone-boundary API (see Phase 0). **Revisit when:** more than one municipality has archaeological-zone KB content - at that point this same city-wide-flag behavior would start producing more frequent false positives, and true neighbourhood-level flagging would need either real geometry data (still unavailable per Phase 0) or richer per-neighbourhood manual_entry content keyed to Nominatim's actual `suburb`/`quarter`/`neighbourhood` fields.
...
**Known, deliberately out-of-scope remaining gap**: `/projects/new` and `/projects/[id]` (the full creation/detail pages, not the dashboard's inline quick-add) are still entirely construction/GIS-flavored (map picker, plot/customer fields) for every vertical. Clicking through to a client's detail page from the new Clients list still shows irrelevant plot-location UI. Not fixed in this pass - flagged in `CAPABILITIES.md` as a known gap so it isn't mistaken for a fresh bug during testing.
